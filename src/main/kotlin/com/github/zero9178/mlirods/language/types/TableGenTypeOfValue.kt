...
private suspend fun typeOfFieldAccess(element: TableGenFieldAccessValueNode): TableGenType {
    ...
    val record = (typeOf(element.valueNode) as? TableGenRecordType)?.record ?: return TableGenUnknownType
    val declaredType = record.fields[identifier]?.typeNode?.toType() ?: return TableGenUnknownType
    ...
    val valueType = coroutineScope { async { TableGenRecordValue(record).fields[identifier].type }.await() }
    return declaredType.refinedBy(valueType)
}
